...
| `CLI_REPO_REF` | `v1.8.3` | Tag (or branch) of the CLI repo to install |
...
4. **Test locally**: Install the CLI locally and run deploy manually to isolate issues:
   ...
   pip install "git+https://<your-pat>@<your-cli-repo>.git@v1.8.3"
...
(e.g. `v1.8.3`). This means the CLI version only changes when you explicitly update it.
...
#    e.g. version = "1.7.7" → version = "1.8.3"

# 3. Tag the release
git tag -a v1.8.3 -m "Release v1.8.3 — description of changes"
...
sed -i "s/vars.CLI_REPO_REF || 'v1.8.3'/vars.CLI_REPO_REF || 'v1.8.3'/g" \
  .github/workflows/*.yml
git add -A && git commit -m "chore: bump CLI to v1.8.3"
git push origin main

# Option B: Set a repo variable (no code change required)
#   Settings → Variables → CLI_REPO_REF = v1.8.3
```

### What Happens If You Forget

- **Nothing breaks** — workflows keep using the pinned tag (e.g. `v1.8.3`)
...
| `CLI_REPO_REF` | `v1.8.3` | CLI version tag to install |

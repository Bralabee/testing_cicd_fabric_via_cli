...
| `CLI_REPO_REF` | `v1.8.0` | Tag (or branch) of the CLI repo to install |
...
4. **Test locally**: Install the CLI locally and run deploy manually to isolate issues:
   ...
   pip install "git+https://<your-pat>@<your-cli-repo>.git@v1.8.0"
...
(e.g. `v1.8.0`). This means the CLI version only changes when you explicitly update it.
...
sed -i "s/vars.CLI_REPO_REF || 'v1.8.0'/vars.CLI_REPO_REF || 'v1.8.0'/g" \
...
- **Nothing breaks** — workflows keep using the pinned tag (e.g. `v1.8.0`)
...
| `CLI_REPO_REF` | `v1.8.0` | CLI version tag to install |

...
> **Generated**: 9 March 2026 -- validated against CLI v1.8.0 and consumer repo template
...
|------|---------------|------------|
| **Platform Engineer** | Maintains the CLI tool (`usf_fabric_cli_cicd`, currently v1.8.0). Pushes version updates, tags releases, and updates the `CLI_REPO_REF` variable in consumer repos. | Develops in the **product repo** (`usf_fabric_cli_cicd`). Never touches consumer configs. |
...
|---|---|---|
| **Product repo** (`usf_fabric_cli_cicd`) | The CLI tool itself. Installed at workflow runtime via `pip install git+...@v1.8.0`. | Python source code, services, utilities, templates. 14 CLI commands: `deploy`, `validate`, `diagnose`, `destroy`, `promote`, `onboard`, `generate`, `list-workspaces`, `list-items`, `bulk-destroy`, `organize-folders`, `init-github-repo`, `scaffold`, `discover-folders`. |
...
| **GitHub Variables** | Settings -> Variables -> Actions | Visible to anyone with repo access | `PROJECT_PREFIX`, `CLI_REPO_REF` (e.g., `v1.8.0`), `FABRIC_CLI_VERSION` |
